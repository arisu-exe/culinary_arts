...
public class BrickOvenScreen extends ContainerScreen<BrickOvenContainer> implements IRecipeShownListener {
    private static final ResourceLocation BRICK_OVEN_GUI_TEXTURE = new ResourceLocation(MainCulinaryArts.MODID, "textures/gui/container/brick_oven.png");
    public final AbstractRecipeBookGui recipeBookComponent;

    public BrickOvenScreen(BrickOvenContainer container, PlayerInventory playerInventory, ITextComponent name) {
    ...
    public void render(MatrixStack stack, int x, int y, float p_230430_4_) {
        ...
        for(Slot slot : this.getMenu().getContainerSlots()) {
            if(slot.hasItem()) {
                ...
                float i = ((float) this.menu.getCurrentBakingTime(slot.index)) / (2f * (float) this.menu.getTargetBakingTime(slot.index));
    ...
    protected void renderBg(MatrixStack stack, float p_230450_2_, int p_230450_3_, int p_230450_4_) {
        this.renderBackground(stack);
        this.minecraft.getTextureManager().bind(BRICK_OVEN_GUI_TEXTURE);
